...
## STRICT CLEAN-ROOM NAMING RULE — APPLY PHASE (MOST CRITICAL ENFORCEMENT POINT)

> **This is the last gate before source names permanently enter gald3r artifacts (task files, PRDs, subsystem specs, TASKS.md). Names written here propagate into code. Enforce strictly.**

Unless the caller explicitly passed `--own-work` or `--allow-source-names`, the following are **FORBIDDEN** in every artifact this skill writes to `.gald3r/`:

| FORBIDDEN in task files, PRDs, subsystem specs, TASKS.md | REQUIRED instead |
|---|---|
| Source project name in task **title** | Descriptive functional title — "Add local diffusion 3D provider" |
| Source project name in PRD **title** | Descriptive PRD title |
| Source project name in subsystem spec **name** | gald3r-native subsystem name |
| Source project name in `## Objective` prose | "an open-weight local model running at zero API cost" — describe capability |
| Exact source class/function/variable names in `## Implementation Notes` | Describe the role and proposed gald3r-native name explicitly |
| Source project name in `title:` YAML field | Descriptive title — this becomes the displayed task name forever |
| Source-named config keys in implementation specs | gald3r-native config key names |
| Source project name in commit messages this skill generates | Descriptive + `Source:` trailer only |

**Provenance fields (ALWAYS ALLOWED):**
- `source:` YAML frontmatter field in task files
- `## Background` section prose — *"the source project implements..."*
- `## License Note` section
- Commit message `Source:` trailer

**Self-check before writing ANY artifact:**
> "Does this task title, PRD title, subsystem name, or Implementation Notes section contain the source project name or exact non-generic source identifiers?"
> If YES — stop, rename to a descriptive gald3r-native equivalent, then write.

**Naming contamination at APPLY time = permanent.** Task titles appear in TASKS.md, commit messages, code comments, and PR descriptions. Once written, they require manual remediation across multiple files.

**Opt-out:** `--own-work` or `--allow-source-names` lifts restrictions. Log reason in APPLY summary.
