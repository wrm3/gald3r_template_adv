...
> **HARD RULE**: Every `TODO[TASK-X→TASK-Y]` annotation written by an implementer during this
> run MUST use a real numeric task ID for `TASK-Y`, obtained by calling `g-skl-tasks CREATE TASK`
> before writing the annotation. The following forms are **FORBIDDEN** and block `[🔍]`:
> `→follow-up`, `→TBD`, `→pending`, `→next`, `→later`, or any non-numeric slug.
> Named-but-not-filed follow-ups are a policy violation. Stubs with placeholder IDs are treated
> as bare TODOs — unmet AC criteria that prevent `[🔍]` marking.
...
| **TODO annotations require real task IDs** -- ` TODO[TASK-X->follow-up] `, ` ->TBD `, ` ->pending `, or any non-numeric slug after ` -> ` are FORBIDDEN; implementers MUST call ` g-skl-tasks CREATE TASK ` before writing the stub annotation and use the returned numeric ID; slug placeholders = bare TODO = unmet AC = blocks `[🔍]` | Prevents undocumented stub debt from shipping untracked |
