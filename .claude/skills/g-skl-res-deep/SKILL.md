...
### STRICT NAMING RULE — Report Outputs (DEFAULT for external sources)

Unless the caller passed `--own-work` or `--allow-source-names`, apply this to every file this skill writes (`01_skeleton.md` through `05_synthesis.md`, `04_FEATURES.md`, vault notes, IDEA_BOARD entries):

| FORBIDDEN in report prose and feature entries | REQUIRED instead |
|---|---|
| Source project name in feature titles or descriptions | Descriptive functional title — "Local diffusion-based 3D generator" not "Hunyuan3D-2 integration" |
| Source org name as an identifier | Generic org role if relevant — "the upstream vendor" |
| Exact non-generic source function/class names in feature descriptions | Describe the role — "the mesh refinement step" not `hunyuan_refine_mesh()` |
| Source-named config keys or CLI flags in feature descriptions | Describe intent — "configure the generation quality level" |

**Provenance fields (ALWAYS ALLOWED):**
- `source:` in YAML frontmatter
- `source_files:` traceability list
- Recon path slugs
- `## License` sections
- `_recon_index.yaml` entries

**Opt-out:** If `--own-work` or `--allow-source-names` was passed by the caller, source names may appear in feature entries. Log the opt-out in `05_synthesis.md` header.

**Self-check before writing any feature entry:**
> "Does this feature title or description contain the source project name or a non-generic source identifier?"
> If YES → replace with a descriptive functional phrase.
