...
### STRICT NAMING RULE — Review and IDEA_BOARD Output (DEFAULT for external sources)

Unless `--own-work` or `--allow-source-names` was explicitly passed, apply this to every IDEA_BOARD entry, review report, and feature staging document this skill writes:

| FORBIDDEN in review output | REQUIRED instead |
|---|---|
| Source project name in IDEA_BOARD `**Title**` | Descriptive functional title — "Local diffusion 3D provider" not "Hunyuan3D-2 provider" |
| Source project name in IDEA_BOARD `**Summary**` prose | Describe the capability — "an open-weight model running locally at zero API cost" |
| Source org name as an identifier in summaries | "the upstream project" or describe role |
| Exact source function/class names in feature summaries | Describe the role — "the mesh refinement stage" |
| Source project name in feature staging doc titles | Descriptive gald3r-native title |

**Provenance fields (ALWAYS ALLOWED):**
- `**Source**:` field in IDEA_BOARD entries (URL goes here)
- `source:` YAML frontmatter
- `## License` sections
- Recon path references

**Before approving `[✅]` on any feature**, additionally check:
> "Does the feature title or IDEA_BOARD entry use the source project name outside a provenance field?"
> If YES → rename before approving. Naming contamination approved at review time locks the name downstream into tasks and code.

**Opt-out:** `--own-work` or `--allow-source-names` lifts naming restrictions. Log opt-out in review summary.
