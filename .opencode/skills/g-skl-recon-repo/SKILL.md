...
### STRICT NAMING RULE — Vault Notes and Summaries (DEFAULT for external sources)

Unless `--own-work` or `--allow-source-names` was explicitly passed, vault notes and summaries written by this skill must NOT use the source project name or exact source identifiers as descriptive labels:

| FORBIDDEN in vault note prose and summaries | REQUIRED instead |
|---|---|
| Source project name in the `title:` frontmatter of the vault note | Descriptive functional title e.g. "Open-weight local 3D diffusion model — [vendor] 2025" |
| Source project name in `## Summary` prose as a feature descriptor | "provides a diffusion-based mesh generation pipeline" |
| Exact source class/API names in vault note body (non-code sections) | Describe the interface role |

**ALWAYS ALLOWED (provenance):**
- `source:` frontmatter field — this is where the URL and repo name live
- `## License` section — license type and source name for attribution
- The vault note **filename** (`research/repos/owner__repo.md`) — slug is traceability
- `_index.yaml` entries — traceability metadata

**When `--deep` is passed**, this skill hands off to `g-skl-res-deep`, which carries its own naming enforcement. Ensure `--own-work` or `--allow-source-names` is forwarded to the deep analysis subagent if the flag was given here.
