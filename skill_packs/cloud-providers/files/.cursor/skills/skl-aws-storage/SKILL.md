...
description: AWS storage — S3 object storage, RDS relational databases, DynamoDB NoSQL, EFS/EBS block/file storage, Glacier archival. Bucket policy templates included.
---
...
Object storage — files, backups, static sites, data lake.
...
# ~50-500% faster from distant regions (Asia → US-East, etc.)
```
...
Serverless NoSQL — single-digit millisecond latency, auto-scaling.
...
```bash
# Global Secondary Index (different partition key — cross-partition queries)
aws dynamodb create-table \
  --table-name Orders \
...
Elastic File System — shared POSIX file storage mountable by multiple EC2/ECS instances.
