...
description: DigitalOcean — Droplets, App Platform, managed PostgreSQL/MySQL/Redis, Spaces object storage, Kubernetes (DOKS), networking. App spec YAML template included.
---
...
Managed PaaS — deploy from Git, Docker, or container registry. No server management.
...
# Pool modes:
# transaction — recommended for web apps (connection freed per transaction)
# session     — connection held until client disconnects (use for LISTEN/NOTIFY)
# statement   — finest granularity (no multi-statement transactions)
```
...
# Key/Secret from: cloud.digitalocean.com/spaces → Manage Keys
