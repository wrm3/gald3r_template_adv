---
name: trellis2-gguf
description: "Trellis 2 GGUF — local image-to-3D pipeline in ComfyUI on 6GB+ VRAM. Generates full 3D meshes with textures + UV unwrap from a single image. Use for characters, props, hard-surface assets, and architectural exteriors. NOT for room interiors, text-to-3D, or scene composition."
tags: [3d, ai, trellis2, gguf, comfyui, image-to-3d, blender, local-ai, low-vram]
---

# Trellis 2 GGUF — Local Image-to-3D

Open-source image-to-3D pipeline running fully locally, free, in ComfyUI. Generates textured 3D meshes with UV unwrap from a single input image. GGUF quantization allows 6GB VRAM GPUs (RTX 3050/4060 class) to run a 4B-parameter model — faster than the full uncompressed version.
...
- Running 3D generation locally on 6–16GB VRAM consumer GPUs

## When NOT to Use

- Interior rooms, floor plans, or scene layouts → use SketchUp or manual modeling
- Text/prompt to 3D → use Meshy, Tripo3D, or 3DAIStudio (cloud)
- Rigging or animation → downstream Blender/Maya step, not this tool
- Production-ready topology → always retopo output in ZBrush, 3DA Studio, or Blender
...
| Q5 | ~7–8 GB | ~7–8 min | 8 GB cards |
| Q6 | ~8–9 GB | ~7 min | 8–10 GB cards |
| **Q8** (best quality/speed) | ~8.9 GB | ~7 min 9s | 8–16 GB cards |
| Full safetensor | ~10–11 GB | ~10 min 8s | 16+ GB only |
...
### Step 1 — Install GGUF Fork

1. Download `install_trellis2_gguf.bat` from `pixel-artistry.com/trellis2gguf`
2. Drop into your ComfyUI add-ons folder
3. Double-click — installs the ArrowX GGUF ComfyUI fork + all dependencies
4. Wait for completion (internet speed dependent, ~5–10 min)
...
4. Grab a coffee — model download takes time

### Step 3 — Load ComfyUI Workflow

1. Launch ComfyUI — ignore any red errors at startup (model path resolution, non-fatal)
2. Load the GGUF workflow from `pixel-artistry.com/trellis2gguf`
3. **Do NOT reuse old Trellis 2 workflows** — GGUF uses different nodes
...
|---|---|---|
| Organic characters | Excellent | Primary use case — fine detail (claws, spikes) preserved at Q4 |
...
Trellis 2 output is a **draft mesh** — production use requires downstream steps:
...
4. **Close everything**: Chrome, Discord, games — every MB counts on 6 GB
5. **Add startup flag**: `--disable-pinned-memory` to ComfyUI startup args (frees held GPU memory)
6. **Combine above**: All three options stack — use together if needed
...
| **TripoAI / Tripo3D** | Cloud service — Trellis 2 is the local equivalent. No text-to-3D. |
| **SketchUp** | No overlap — SketchUp is parametric/architectural, Trellis 2 is object-from-image. Use together. |
