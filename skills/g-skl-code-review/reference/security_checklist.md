...
Use for architectural changes, new services, or cross-process boundaries.

| Threat | What to Check |
|--------|---------------|
| **S**poofing | Can an attacker impersonate a user, service, or API? Are identity proofs (tokens, certs, signatures) verified? |
| **T**ampering | Can data be modified in transit or at rest without detection? Are integrity checks (MACs, signatures, checksums) present? |
| **R**epudiation | Can users deny performing actions? Is there an audit log for sensitive operations (login, data changes, payment)? |
| **I**nformation Disclosure | Can PII, secrets, or internal state leak via logs, error messages, stack traces, or debug endpoints? |
| **D**enial of Service | Are there resource limits on inputs (file size, request rate, query depth, loop bounds) that prevent exhaustion attacks? |
| **E**levation of Privilege | Can an unprivileged user gain admin/root access through a flaw in role checks, token validation, or privilege escalation paths? |

### STRIDE Checklist

- [ ] **Spoofing** — all identity claims verified; no implicit trust based on header alone
- [ ] **Tampering** — integrity checks on critical data; signed payloads where needed
- [ ] **Repudiation** — audit log entries for sensitive mutations (create/delete/financial)
- [ ] **Information Disclosure** — no secrets/PII in logs, error responses, or URLs
- [ ] **DoS** — rate limits, max payload size, and query depth caps in place
- [ ] **Elevation of Privilege** — role checks enforced at each layer; no client-dictated privilege

### False Positive Handling

Add `# nosec: <justification>` on the same line or `# security-exempt: <reason>` as a comment block above the flagged code. Document the business or technical reason clearly. Annotated exemptions are waived in code review security reports.
