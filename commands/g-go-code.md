...
**b3.5) Completion Check - Definition-of-Done Evaluator (T1099)** - after implementation, before marking `[??]`, run a cheap-model evaluation of whether the task AC was met.

**Evaluator prompt template** (invoke with cheapest available model: Haiku, gemini-flash-lite, etc.):
```
Task acceptance criteria:
{paste task ## Acceptance Criteria checkboxes}

Implementation summary:
{brief summary of what was done -- 3-5 sentences max}

Question: Were ALL acceptance criteria above met by this implementation?
Answer: YES or NO.
If NO, list each unmet criterion on a separate line.
```

**Loop behavior**:
- If evaluator says **YES** -> proceed to b4 Handoff Report -> `[??]`
- If evaluator says **NO** -> do one targeted fix pass, then re-evaluate (max 1 retry)
- If max retries reached or no cheap model available -> proceed with a note: `completion_check: SKIPPED (model unavailable)`

**Status History entry** (append after evaluation):
```
| {timestamp} | in-progress | awaiting-verification | {agent} | completion_check: {YES/NO/SKIPPED} - {unmet criteria or 'all met'} |
```
