...
8.5. **Offer to initialize gald3r_muninn graph index** (T1159, optional):
   The muninn codebase knowledge graph indexer (Python AST + TypeScript) feeds `g-go-code` Step b0 Impact Scan. Initializing the index is optional — the indexer is reachable through the `gald3r_valhalla` MCP server.

   **Gate**: only offer this step when `gald3r_valhalla` MCP is reachable from this machine (probe `http://localhost:8090/mcp` or read `.mcp.json` for the configured URL). If unreachable, skip silently and add a hint to the printed next steps: `"Run @g-skl-muninn ANALYZE later once the MCP server is up."`

   When the gate passes, offer (default N for cost-of-init transparency):
   ```
   "Initialize gald3r_muninn graph index now? Indexes Python + TS files into ~/.gald3r/muninn.db. Takes 1-5 minutes for a typical repo. [y/N]"
   → Y: invoke muninn `graph_status` to check existing index, then run incremental `analyze` if empty; install the post-commit hook (`.git/hooks/post-commit` → `.<ide>/hooks/g-hk-graph-update.ps1`) so future commits keep the index fresh.
   → N: print "Run @g-skl-muninn ANALYZE later when ready" and continue.
   ```

   Notes:
   - The post-commit hook is non-blocking: it runs the incremental indexer but exits 0 even when the indexer is unavailable, so commits never fail because of muninn.
   - The same hook file (`g-hk-graph-update.ps1`) is parity-shipped to all 5 IDE hook directories (`.cursor/`, `.claude/`, `.agent/`, `.codex/`, `.opencode/`); pick the one that matches the user's primary IDE when wiring the `.git/hooks/post-commit` entry.
   - This step replaces the legacy `g-hk-gitnexus-update.ps1` hook, which is retained as a deprecated forwarding shim.
