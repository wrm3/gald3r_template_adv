---
name: skl-telegram
description: Telegram channel & community management — bots, channels, groups, moderation, notifications, growth, Mini Apps. GitHub/CI webhook integration.
---
...
### BotFather workflow
1. Open @BotFather → `/newbot` → choose name + username (must end in "bot")
...
- **Public**: searchable, `t.me/username` — for announcements
- **Private**: invite-link only — for beta testers
...
scheduler.add_job(
    lambda: bot.send_message(chat_id="@mychannel", text="📢 Weekly update!"),
...
| Subscriber growth | — | TGStat/Telemetr |

---

## GROUP — Supergroup Administration
...
## NOTIFICATIONS — GitHub & CI Integration
...
- name: Telegram notify
  run: |
    curl -s -X POST "https://api.telegram.org/bot${{ secrets.TG_TOKEN }}/sendMessage" \
      ...
      -d text="$([[ '${{ job.status }}' == 'success' ]] && echo '✅' || echo '❌') <b>Build ${{ job.status }}</b>"
```
...
async def gald3r_task_alert(task_id: int, title: str, status: str):
    ...
    icon = "✅" if status == "completed" else "🔍" if status == "awaiting-verification" else "❌"
...
- **TGStat.ru** — `https://tgstat.ru/en/channel/add` — submit username, verify ownership via pinned post or bot
- **Telemetr.io** — `https://telemetr.io/en/channels` — automatic index for public channels >100 subscribers
- **Combot Ratings** — auto-indexed via @ComBot when added to your channel
...
```python
# 1. Get Stripe test token from @BotFather → /mybots → Payments → Connect Stripe (test)
# 2. Send invoice
...
- No per-user analytics — subscriber count + post views only
