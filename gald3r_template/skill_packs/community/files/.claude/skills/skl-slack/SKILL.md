---
name: skl-slack
description: Slack workspace management — channels, apps, webhooks, workflow builder, bot development with Bolt SDK, Block Kit UI, MCP context gaps, community management.
---
...
- For app development: https://api.slack.com/apps → Create New App
...
**App manifest (api.slack.com → App Manifest):**
```yaml
display_information:
  name: gald3r-bot
...
**Useful workflows:**
- **Standup collector**: Scheduled message asking team for updates → compiles thread
- **Incident response**: `/incident` command → creates channel, pages on-call, posts runbook
- **Onboarding**: New member joins → DMs welcome message with links
- **Approval flow**: Request → manager DM → approve/deny → channel notification

Setup: Workspace → Automations → Workflow Builder → Create Workflow
...
**Workarounds:**
1. Use Slack's "Save to gald3r" workflow (Workflow Builder + webhook → vault note)
2. Export channel history via Slack API → `vault_sync` MCP tool
3. Use Slack's Zapier/Make integration → POST to gald3r webhook endpoint
...
- **User groups**: `@design-team`, `@on-call` — notify subsets without @channel
...
- **Moderation**: Workspace settings → Members → Restrict messaging; DM guard for spam
