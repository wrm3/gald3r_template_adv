...
description: Cloudflare DNS & domain management — zones, records, SSL/TLS, page rules, wrangler CLI, bulk operations. Foundation for Workers and Tunnels skills.
---
...
| Setting | Value | Notes |
|---------|-------|-------|
| max_age | 31536000 | 1 year — required for preload |
...
## PAGE-RULES — URL-Based Overrides (Legacy)
...
|--------|--------|----------------|
| API Token | `Authorization: Bearer $CF_API_TOKEN` | **Preferred** — scoped |
| API Key (legacy) | `X-Auth-Email` + `X-Auth-Key` | Avoid — full access |
