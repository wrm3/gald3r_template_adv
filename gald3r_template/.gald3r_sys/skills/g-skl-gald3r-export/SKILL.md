...
> **MAINTAINER TOOL** — This skill exists only under gald3r_dev root IDE configs (`.cursor/`, `.claude/`, `.agent/`, `.codex/`, `.opencode/`). It is **never** copied into `G:/gald3r_ecosystem/gald3r_template_full` nor propagated to consumer installs.
...
`g-skl-gald3r-export` produces a **standalone, clonable folder** from `G:/gald3r_ecosystem/gald3r_template_full` contents — ready for `git init`, commit, and release. It is the guarded publish path for shipping the gald3r framework to consumers.
...
- `<dest>/` — full `G:/gald3r_ecosystem/gald3r_template_full` tree (all IDE dirs, `.gald3r/`, docs)
...
- **Never** in `G:/gald3r_ecosystem/gald3r_template_full` — this would create a circular self-reference
