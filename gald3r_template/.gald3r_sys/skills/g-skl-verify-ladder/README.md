...
Multi-level verification gates for autonomous task completion. Configurable levels from minimal (lint) to thorough (tests + acceptance + hallucination guard).

## When to use

- Invoke via `@g-skl-verify-ladder` (or when the agent determines this skill is relevant)
