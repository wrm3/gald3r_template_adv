...
# ── Idempotency guard ─────────────────────────────────────────────────────────
if (-not $ForceRun -and $env:GALD3R_HK_VAULT_MIGRATE_APPLIED -eq "1") {
    Write-Host "[SKIP] g-hk-vault-migrate already applied this session. Pass -ForceRun to override."
    exit 0
}
$env:GALD3R_HK_VAULT_MIGRATE_APPLIED = "1"
