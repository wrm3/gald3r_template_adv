# OpenHands — gald3r Deploy Scaffold

**Config folder**: `.openhands/`

This directory is the gald3r deploy scaffold for **OpenHands**. It is registered in
`.gald3r_sys/_platform_capabilities.json` and recognised by the platform-parity
sync tooling.

Authoritative install + customization guide: **`g-skl-platform-openhands`** (.gald3r_sys/skills/g-skl-platform-openhands/SKILL.md).

> **Status (T1207 → T1277):** the platform skill and capabilities-registry entry
> are in place. The full per-platform deploy payload (instructions guide + config
> files such as hooks/settings/conventions) is tracked as a follow-up in **T1277**.
> Until then this README is the placeholder that keeps the scaffold git-tracked.
