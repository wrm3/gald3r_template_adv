...
```
Landing:  ΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûê  (wide, flat = squash)
Idle:     ΓûêΓûêΓûêΓûêΓûêΓûê    (normal)
Jumping:  ΓûêΓûêΓûêΓûê      (tall, thin = stretch)
          ΓûêΓûê
```
...
```
Timeline: [anticipation 3-5f] ΓåÆ [action 2-4f] ΓåÆ [follow-through 3-6f]
```
...
**Test**: Silhouette test ΓÇö can you identify the action from a black silhouette alone?

### 4. Straight Ahead vs Pose-to-Pose
**Straight Ahead**: Animate frame by frame (organic, fluid ΓÇö fire, smoke, hair)
**Pose-to-Pose**: Define key poses, fill in between (controlled ΓÇö character actions)
...
```
Character stops walking ΓåÆ body stops ΓåÆ hair swings forward ΓåÆ settles back
```
...
**Our toon-shaded style**: Already exaggerated visuals ΓåÆ match with exaggerated animation. Subtle realism would look wrong.
...
- Sindri: Minimal movement, deliberate
- Brokkr: Exaggerated reactions
- Odin: Grand gestures, takes up space
...
Blend time guidelines:
  Idle ΓåÆ Walk:     0.2-0.3s
  Walk ΓåÆ Run:      0.2s
  Walk ΓåÆ Idle:     0.3-0.4s
  Any ΓåÆ Sit:       0.5s (longer = more natural)
  Sit ΓåÆ Stand:     0.4s
  Any ΓåÆ Talk:      0.2s
  Talk ΓåÆ Idle:     0.3s
```
