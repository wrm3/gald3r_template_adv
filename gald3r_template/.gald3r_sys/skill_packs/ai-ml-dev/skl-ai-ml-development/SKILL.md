...
description: AI/ML training through deployment ΓÇö fine-tuning, RLHF, multimodal systems, RAG, memory, MLOps, and tool integration.
version: 1.0.0
skill_group: "ai-ml-dev"
skill_category: "AI & Machine Learning Development"
---
...
Γö£ΓöÇΓöÇ Yes: Do you have labeled data?
Γöé   Γö£ΓöÇΓöÇ Yes (>10k): Full fine-tuning or LoRA
Γöé   Γö£ΓöÇΓöÇ Yes (1k-10k): LoRA/QLoRA
Γöé   ΓööΓöÇΓöÇ No: Generate synthetic data or use few-shot
ΓööΓöÇΓöÇ No: Start with prompting, iterate to fine-tuning
```
...
| Data | Preferences ΓåÆ Reward model | Direct preferences |
...
```
Γûí Quantization
  Γûí INT8 (2x memory reduction, ~5% quality loss)
  Γûí INT4/AWQ/GPTQ (4x reduction, ~10% quality loss)
  Γûí GGUF for CPU inference

Γûí Inference Optimization
  Γûí KV-cache optimization
  Γûí Continuous batching
  Γûí Speculative decoding (if supported)
  Γûí Flash Attention 2

Γûí Serving Configuration
  Γûí Appropriate batch size
  Γûí Request queuing
  Γûí Timeout handling
  Γûí Health checks

Γûí Monitoring
  Γûí Latency (P50, P95, P99)
  Γûí Throughput (tokens/sec)
  Γûí Error rate
  Γûí GPU utilization
  Γûí Memory usage
```
