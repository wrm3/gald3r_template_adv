...
description: Blockchain devΓÇöEVM, Solana, Cosmos, Bitcoin; Solidity, DeFi, NFTs, tokens, cross-chain.
version: 1.0.0
skill_group: "blockchain"
skill_category: "Blockchain & Web3"
---
...
```
Γûí Reentrancy Protection
  Γûí Use ReentrancyGuard or checks-effects-interactions
  Γûí External calls last
  Γûí State changes before external calls

Γûí Access Control
  Γûí onlyOwner or role-based access
  Γûí Multi-sig for critical functions
  Γûí Timelock for governance

Γûí Input Validation
  Γûí Check zero addresses
  Γûí Validate array lengths
  Γûí Bound numeric inputs

Γûí Integer Safety
  Γûí Solidity 0.8+ (built-in overflow checks)
  Γûí Use SafeMath for older versions

Γûí External Calls
  Γûí Check return values
  Γûí Handle failed calls
  Γûí Limit gas for calls

Γûí Upgrades
  Γûí Storage layout compatibility
  Γûí Initializer protection
  Γûí Upgrade authorization
```
...
// Γ¥î Expensive storage writes
function bad() public {
    counter = counter + 1;  // SSTORE every time
    counter = counter + 1;
}

// Γ£à Batch storage writes
...
```
Γûí Pre-Deployment
  Γûí All tests passing
  Γûí Code audited (if handling value)
  Γûí Gas optimized
  Γûí Verified on testnet

Γûí Deployment
  Γûí Correct network selected
  Γûí Sufficient gas/funds
  Γûí Constructor args verified
  Γûí Transaction confirmed

Γûí Post-Deployment
  Γûí Verify on block explorer
  Γûí Test basic functions
  Γûí Transfer ownership (if applicable)
  Γûí Update frontend/docs with addresses

Γûí Security
  Γûí Renounce unnecessary permissions
  Γûí Set up multi-sig (if applicable)
  Γûí Monitor for unusual activity
```
